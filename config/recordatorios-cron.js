...
async function ejecutarRecordatorios() {
  ...
  try {
    ...
    for (const cita of citasProximas) {
      ...
      if (cita.enviar_whatsapp && telefonoDestino) {
        try {
          ...
          console.error(`❌ Error enviando WhatsApp a ${telefonoDestino}:`, wsError.message);
        }
      }
      ...
      // 3. CORRECCIÓN CLAVE: Esperar a que Laravel actualice antes de continuar o cerrar
      try {
    ...
    console.log('🏁 Proceso finalizado con éxito. Cerrando contenedor...');
    process.exit(0); // Apaga el script limpiamente SOLO cuando todo terminó
